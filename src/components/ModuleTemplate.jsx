...
import { ArrowLeft, Plus, Edit, Trash2, Save, X, AlertCircle, CheckCircle, Moon, Sun, ExternalLink, FileText, MessageSquare } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <div className="flex justify-between items-center">
            <h1 className="text-4xl font-bold text-gray-800 dark:text-white">{moduleName}</h1>
            <div className="flex gap-2">
              <button
                onClick={toggleTheme}
                className="bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-yellow-400 p-2 rounded-lg transition-colors"
                title="Cambiar tema"
              >
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 text-white px-4 py-2 rounded-lg flex items-center transition-colors"
              >
        ...
        {/* ── Banner de error ── */}
        {errorMsg && (
          <div className="mb-4 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 text-red-800 dark:text-red-200 rounded-lg px-4 py-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm mt-1 font-mono bg-red-100 dark:bg-red-900/40 rounded p-1">{errorMsg}</p>
                {isRLSError && (
                  <div className="mt-3 text-sm bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded p-3 text-yellow-900 dark:text-yellow-200">
...
ALTER TABLE student_modules DISABLE ROW LEVEL SECURITY;`}
              ...
              <button onClick={() => setErrorMsg(null)} className="text-red-400 dark:text-red-300 hover:text-red-600 dark:hover:text-red-400 flex-shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        {/* ── Banner de éxito ── */}
        {successMsg && (
          <div className="mb-4 flex items-center gap-3 bg-green-50 dark:bg-green-900/20 border border-green-300 dark:border-green-700 text-green-800 dark:text-green-200 rounded-lg px-4 py-3">
            ...
            <form onSubmit={handleSubmit}>
                  ...
                  {field.type === 'file' ? (
                    <div className="flex gap-2">
                      ...
                      <label
                        ...
                        className="flex-1 px-4 py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-center cursor-pointer hover:border-blue-500 dark:hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors dark:bg-gray-700"
                      >
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  disabled={saving}
                  className="bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 disabled:bg-green-400 disabled:cursor-not-allowed text-white px-6 py-2 rounded-lg flex items-center transition-colors"
                >
                  {saving ? (
                    <>
                ...
                <button
                  ...
                  onClick={resetForm}
                  disabled={saving}
                  className="bg-gray-500 hover:bg-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition-colors"
                >
                  Cancelar
                </button>
              </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-semibold dark:text-white flex items-center">
              <ExternalLink className="w-6 h-6 mr-2 text-blue-600 dark:text-blue-400" />
              Lista de {moduleName}
            </h2>
            <button
              onClick={fetchData}
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-lg dark:hover:shadow-xl dark:hover:shadow-gray-900 transition-all duration-200 bg-gradient-to-r from-white to-gray-50 dark:from-gray-800 dark:to-gray-700/50 hover:scale-[1.02]">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="flex-shrink-0">
                      {item.content.imagen ? (
                        ...
                        <div className="w-32 h-32 flex items-center justify-center bg-gray-200 dark:bg-gray-600 rounded-lg border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="flex-1">
                      {fields.map((field) => {
                        ...
                        // Enlace como botón clickeable
                        if (field.name === 'enlace') {
                          return (
                            <div key={field.name} className="mb-2">
                              <span className="font-medium text-gray-700 dark:text-gray-300 flex items-center">
                                <ExternalLink className="w-4 h-4 mr-1" /> {field.label}:
                              </span>
                              {item.content[field.name] ? (
                                <a 
                                  href={item.content[field.name]} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline inline-block ml-5"
                                >
                                  Ir al enlace →
                                </a>
                              ) : (
                                <span className="text-gray-600 dark:text-gray-400 ml-5">N/A</span>
                              )}
                            </div>
                          );
                        }
                        ...
                        // Concepto con ícono
                        if (field.name === 'concepto') {
                          return (
                            <div key={field.name} className="mb-2">
                              <span className="font-medium text-gray-700 dark:text-gray-300 flex items-center">
                                <FileText className="w-4 h-4 mr-1" /> {field.label}:
                              </span>
                        ...
                        if (field.name === 'descripcion') {
                          return (
                            <div key={field.name} className="mb-2">
                              <span className="font-medium text-gray-700 dark:text-gray-300 flex items-center">
                                <MessageSquare className="w-4 h-4 mr-1" /> {field.label}:
                              </span>
                              <span className="text-gray-600 dark:text-gray-400 ml-5">
                                {item.content[field.name] || 'N/A'}
                              </span>
